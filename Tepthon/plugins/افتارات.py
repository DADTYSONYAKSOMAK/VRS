...
from VR_11_V.core.logger import logging
...
async def _(event):
    ...
    try:
        ...
        await event.client.send_file(
            event.chat_id,
            file=random.choice(zedph),
            caption=f"**🎆┊صـورة ومعلومـة ➧ 🛤💡◟**\n\n[➧??𝙤𝙪𝙧𝙘𝙚 𝙏𝙀𝙋𝙏𝙃𝙊𝙉](https://t.me/VR_11_V)",
        )
        await zzevent.delete()
    except Exception:
        await zzevent.edit("**╮•⎚ عذرًا .. لـم استطـع ايجـاد المطلـوب ☹️💔**")
...
async def _(event):
    ...
    try:
        zedre = [
            zlzz7
            async for zlzz7 in event.client.iter_messages(
                "@VR_SOURCE_3", filter=InputMessagesFilterPhotos
            )
        ]
        aing = await event.client.get_me()
        await event.client.send_file(
...
async def _(event):
    ...
    try:
        zedph = [
            zelzal
            async for zelzal in event.client.iter_messages(
                "@VR_SOURCE_2", filter=InputMessagesFilterPhotos
            )
        ]
        aing = await event.client.get_me()
        await event.client.send_file(
