...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**⛥ ⤻ انتـظࢪ جـاࢪي فـحص بـ ـوت  𝗩𝞝𝗥  الخـاص بـِك   ۦ**")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "☼ ⤶"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** ☼ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝞝𝗥 ‌‌‏𓅓 . **"
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "☼ ⤶"
    zed_caption = "** ☼ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝞝𝗥 ‌‌‏𓅓 . **\n"
    zed_caption += f"**{Z_EMOJI} َTEᒪETᕼOᑎ 𓋪** `{version.__version__}\n`"
    zed_caption += f"**{Z_EMOJI}  𝗩𝞝𝗥  𓋪 :** `{zedversion}`\n"
