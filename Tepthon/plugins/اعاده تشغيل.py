...
async def _(event):
    ...
    sandy = await edit_or_reply(
        ...
        f"**•⎆┊يتـم الان اعـادة تشغيـل بـوت VR قـد يستغـرق الامـر 1-2 دقيقـة ▬▭ ...**",
    )
    try:
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "لـ إطفـاء الداينـو الخاص بتنصيبك بهيروكـو .. لا تستطيع اعادة التشغيل مرة اخرى عبر حسابك عليك الذهاب لحساب هيروكو واتباع الشرح التالي https://t.me/VR_11_V",
...
async def _(event):
    "لـ إطفـاء البـوت"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "#ايقــاف البــوت\n\n" "**- بـوت VR فـي وضــع الايقــاف**")
    await edit_or_reply(event, "**✾╎جــارِ إيقـاف تشغيـل بـوت VR الآن 📟 ...**\n\n**✾╎شغِّـلنـي يـدويًـا لاحقًــا**\n**✾╎باتبـاع الشـرح** https://t.me/VR_11_V")
