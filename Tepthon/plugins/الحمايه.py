...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""ᯓ𝗦𝗢𝗨𝗥𝗖𝗘 𝗩𝞝𝗥   **- الـرد التلقـائي 〽️**
**•─────────────────•**
...
async def do_pm_request_action(event, chat):
    ...
    the_message = f"#حمـايـة_الخـاص\
                \n** ⎉╎المستخـدم** [{get_display_name(chat)}](tg://user?id={chat.id}) .\
                \n** ⎉╎تم حظـره .. تلقائــيًا**\
                \n** ⎉╎السـبب:** لقد اختار خيار الطلب ولكنه لم ينتظر وتم حظره تلقائـيـًا 🥲😹."
...
async def do_pm_chat_action(event, chat):
    ...
    the_message = f"#حمـايـة_الخـاص\
                \n** ⎉╎المستخـدم** [{get_display_name(chat)}](tg://user?id={chat.id}) .\
                \n** ⎉╎تم حظـره .. تلقائيًـا**\
                \n** ⎉╎السـبب:** لقد اختار خيار الدردشـه مع المالك ولكنه لم ينتظر وتم حظره تــلـقائيـًا 🥲😹."
...
async def do_pm_spam_action(event, chat):
    ...
    the_message = f"#حمـايـة_الخـاص\
                            \n** ⎉╎المستخـدم** [{get_display_name(chat)}](tg://user?id={chat.id}) .\
                            \n** ⎉╎تم حظـره .. تلقائيًا**\
                            \n** ⎉╎السـبب:** لقد اختار خيار ازعاج المالك واستمر بتكرار الرسائل وتم حظره تلقائيـًا 🥲😹."
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5847230349 or event.chat_id == 5847230349 or event.chat_id == 5847230349 or event.chat_id == 5847230349:
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**⤶ حـسـنـاً عـزيـزي ، تـم إرسـال طـلـبـڪ بـنـجـاح 📨 . لا تـقـم بـ إخـتـيـار خـيـار آخــر .**\n**⤶ سيـتـم الـرد عـلـيـڪ عـنـد تـفـرغ الـمـالـڪ .🧸🤍**"
